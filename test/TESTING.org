...
Vbuf uses a native browser-based test suite with no dependencies on Jest or NPM.
...
- *Right panel*: Live Vbuf editor showing the test execution
...
Each test gets a *fresh DOM node and Vbuf instance* via ~new EditorTestHarness()~. This ensures proper test isolation with no state leakage between tests.

** EditorTestHarness & Literate DSL

The test harness serves three purposes:

1. *Test isolation* - Provides an isolated editor instance (DOM node + Vbuf) for each test
...
// Access Vbuf instance and DOM node
fixture.wb    // Vbuf instance
